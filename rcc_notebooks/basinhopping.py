from typing import Callable, Iterable, List, Tuple, Optional, TypeVar, Dict
...
class Optimize:
    def basinhopping(
        ...
        **scipy_kwargs,
        ...
        """
        Minimize is a wrapper around the `scipy.basinhopping` function
        ...
        Args:
            ...
            step_callback: If not None, a callable that gets called once after
                ...
                step.
            niter: The number of iterations that the
        ...
        if stepsize is not None:
            scipy_kwargs.update(dict(stepsize=stepsize))
            
        if niter is not None:
            scipy_kwargs.update(dict(niter=niter))
            
        if T is not None:
            scipy_kwargs.update(dict(T=T))
            
        
        if minimizer_kwargs is None:
            raise ValueError('Good sir or madam, you twerp! You forgot the minimizer kwargs.')
    

        return scipy.optimize.basinhopping(
        func, initial_params, minimizer_kwargs=minimizer_kwargs, **scipy_kwargs)
